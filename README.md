...
현재 배포 주소는 `https://heny101.github.io/barista-study/`입니다. Android Chrome에서는 브라우저의 **앱 설치** 또는 **홈 화면에 추가**를 사용하고, iPhone/iPad Safari에서는 **공유 → 홈 화면에 추가**를 선택해 설치할 수 있습니다.

처음 온라인으로 앱을 열어 필요한 파일과 문제 데이터를 캐시한 뒤에는 기본적인 오프라인 학습이 가능합니다. 학습 기록은 해당 브라우저 또는 설치 앱의 `localStorage`에 저장되며, 동일 origin에서는 기존 기록이 유지됩니다. 브라우저의 사이트 데이터를 삭제하면 기록도 삭제될 수 있습니다.
